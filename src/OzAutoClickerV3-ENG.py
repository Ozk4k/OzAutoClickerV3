...
class AutoClickerGUI:
    ...
    def setup_gui(self):
        ...
        for text, val in [
            ("⏱️ Timer (5s countdown)", "timer"),
            ("🖱️ User click selection", "click"),
            ("🎯 Follow cursor in real-time", "follow")
        ]:
        ...
        tk.Label(keys_frame, text="⌨️ Hotkey Configuration:", bg=self.colors['card'],
                 fg=self.colors['fg'], font=('Arial', 10, 'bold')).pack(anchor='w', padx=15, pady=(10, 5))
        ...
        tk.Label(start_key_frame, text="▶️ Start Key:", bg=self.colors['card'],
        ...
        tk.Button(start_key_frame, text="🔧 Change",
        ...
        tk.Label(stop_key_frame, text="⏹️ Stop Key:", bg=self.colors['card'],
        ...
        tk.Button(stop_key_frame, text="🔧 Change",
        ...
        tk.Label(control_frame, text="⏱️ Click Interval:", bg=self.colors['card'],
                 fg=self.colors['fg'], font=('Arial', 10, 'bold')).pack(anchor='w', padx=15, pady=(10, 5))
        ...
        # Control buttons
        tk.Button(frame, text="▶️ Start AutoClicker", command=self.start_clicking,
                  bg=self.colors['success'], fg='white',
                  font=('Arial', 12, 'bold'),
                  relief='flat', bd=0, highlightthickness=0,
                  pady=10).pack(fill='x', pady=(20, 5), padx=5)

        tk.Button(frame, text="⏹️ Stop AutoClicker", command=self.stop_clicking,
        ...
        self.hotkey_status = tk.Label(frame, 
                                      text=f"📌 Active hotkeys: {self.start_key.get()} (start) | {self.stop_key.get()} (stop)",
    ...
    def set_position_timer(self):
        ...
        def countdown():
            for i in range(5, 0, -1):
                self.position_label.config(text=f"⏱️ Waiting... {i}s", fg=self.colors['warning'])
                self.root.update()
                time.sleep(1)
            pos = pyautogui.position()
            self.click_position = pos
            self.position_label.config(text=f"✅ Position: ({pos.x}, {pos.y})", fg=self.colors['success'])
            self.pos_button.config(state="normal")
    ...
    def set_position_click(self):
        self.pos_button.config(state="disabled")
        self.position_label.config(text="🖱️ Click anywhere on the screen...",
        ...
        def wait_for_click():
            ...
            if pos:
                ...
                self.position_label.config(text=f"✅ Position: {pos[0]}", fg=self.colors['success'])
            else:
                self.position_label.config(text="❌ Error capturing click", fg=self.colors['danger'])
    ...
    def set_follow_mode(self):
        ...
        self.position_label.config(text="🎯 Mode: Following cursor", fg=self.colors['success'])
    ...
    def start_clicking(self):
        ...
        if not self.click_position:
            messagebox.showwarning("⚠️ Warning", "Please set a position first!")
            return
        self.clicking = True
...
if __name__ == "__main__":
    print("🖱️ OzAutoClickerV3 - English Edition")
    print("Dependencies: pip install pyautogui keyboard pynput")
    print("💡 Press ESC during key capture to cancel")
    main()